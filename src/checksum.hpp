...
*/
template <typename T>
T calculateFletcher(const std::string & filePath, const uint64_t dataSize = 0, T checksum = 1)
{
	//open file
	...
	if (inStream.is_open() && inStream.good())
	{
		...
		uint64_t rollingSize = 0;
		while (!inStream.eof() && inStream.good())
		{
			uint8_t buffer[4096];
			uint64_t readSize = sizeof(buffer);
			try
			{
...
*/
template<typename T>
T calculateFletcher(const uint8_t * data, const uint64_t dataSize, T checksum = 1);
